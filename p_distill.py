...
class AdapterMTT6:
    ...
    def distill(
        ...
        if enable_adapter and adapter_dict:
            ...
            student_net.train()
            # Build a flat param vector but keep adapter and base parts as separate nn.Parameters
            flat_param_list = []
        ...
        with tqdm(range(0, num_iteration)) as pbar:
            for distill_it in pbar:
                ...
                end_target_expert_params = []
                last_key = sorted(expert_file_list.keys())[-1]
                # last_value = expert_file_list[last_key]
                load_net.load_state_dict(torch.load(expert_file_list[last_key], weights_only=False)['model_state_dict'])
                for param in load_net.parameters():
                    end_target_expert_params.append(param.data.reshape(-1))
                end_target_expert_params = torch.cat(end_target_expert_params, dim=0).to(device)
                ...
                for batch_data in tqdm(dataloder, disable=True):
                    ...
                    if distill_energy and distill_force:
                        loss = energy_criterion(output, batch_data.y)
                ...
                """
                先算 outloss 因为 outloss 涉及到 base param 的计算图
                """
                preds = torch.Tensor([])
                ...
                zeros_full = assemble_flat_params(adapter_params=None, base_params=next_base,
                                                  base_mask=base_model_params_mask)
                load_net.eval()
                for batch_data in tqdm(dataloder, disable=True):
                    batch_data = batch_data.to(device)
                    ...
                    out = load_net(batch_data)
                    force = -torch.autograd.grad(outputs=out, inputs=batch_data.pos,
                                                 grad_outputs=torch.ones_like(out),
                                                 create_graph=True, retain_graph=False)[0]
                    sout = student_net(batch_data, flat_param=zeros_full)
                    sforce = -torch.autograd.grad(outputs=sout, inputs=batch_data.pos,
                                                  grad_outputs=torch.ones_like(out),
                                                  create_graph=True, retain_graph=False)[0]

                    preds_force = torch.cat([preds_force, sforce.detach().cpu()], dim=0)
                    targets_force = torch.cat([targets_force, force.detach().cpu()], dim=0)
                    torch.cuda.empty_cache()
                    preds = torch.cat([preds, sout.detach().cpu()], dim=0)
                    targets = torch.cat([targets, out.detach().cpu()], dim=0)
                ...
                outloss = energy_mae + p * force_mae
                """
                第二算 param loss
                """
                ...
                if base_model_params_mask is not None:
                    ...
                    param_loss += torch.nn.functional.mse_loss(student_params_list[-1][base_model_params_mask],
                                                               end_target_expert_params, reduction="sum")
                    param_dist += torch.nn.functional.mse_loss(start_expert_params,
                                                               end_target_expert_params, reduction="sum")
                else:
                    param_loss += torch.nn.functional.mse_loss(student_params_list[-1], end_target_expert_params,
                                                               reduction="sum")
                    param_dist += torch.nn.functional.mse_loss(start_expert_params, end_target_expert_params,
                                                               reduction="sum")
                    # 0-250  distill - 1.
                ...
                if self.optimizer_adapter is not None:
                    ...
                    # param_loss.backward(retain_graph=True)
                    totalloss.backward()
