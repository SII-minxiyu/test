...
class AdapterMTT3:
    ...
    def distill(
        ...
        param_loss_list = []
        adapter_params_list = []
        with tqdm(range(0, num_iteration + 1)) as pbar:
            for distill_it in pbar:
                ...
                sampler.set_indices(np.asarray(data_indices_list).tolist())
                ...
                for batch_data in tqdm(dataloder, disable=True):
                    ...
                    if adapter_params is not None:
                        student_params_list[-1].scatter_(0, torch.nonzero(~base_model_params_mask).squeeze(),
                                                         adapter_params)
                    output = student_net(batch_data, flat_param=student_params_list[-1])
                    if distill_energy and distill_force:
                        ...
                        force = - \
                            torch.autograd.grad(outputs=output, inputs=batch_data.pos,
                    ...
                    if base_model_params_mask is not None:
                        next_student_params = dynamic_optimizer.step(
                            params=student_params_list[-1][base_model_params_mask], grad=grad[base_model_params_mask],
                            lr=dynamic_scheduler.step_lr(dynamic_optimizer.get_lr()))
                    else:
                        next_student_params = dynamic_optimizer.step(params=student_params_list[-1], grad=grad,
                                                                     lr=dynamic_scheduler.step_lr(
                                                                         dynamic_optimizer.get_lr()))
                    ...
                    if adapter_params is not None:
                        ...
                        student_params_list.append(next_student_params)
                    del output, grad
                ...
                # 用 zeros 把 adapter 部分“屏蔽掉”，同时保持 base 部分的计算图
                zeros_full = torch.zeros_like(final_flat)
                # outer_flat : base 部分来自 final_flat（保留计算图），adapter 部分为 0（常数）
                ...
                for batch_data in tqdm(dataloder, disable=True):
                    ...
                    if distill_force:
                        batch_data.pos.requires_grad_(True)
                    # only_base = torch.zeros_like(student_params_list[-1])
                    # only_base.scatter_(0, torch.nonzero(~base_model_params_mask).squeeze(), 0)
                    # only_base.scatter_(0, torch.nonzero(base_model_params_mask).squeeze(), next_student_params)
                    outputstu = student_net(batch_data, flat_param=outer_flat)
                    forcestu = -torch.autograd.grad(
                        outputs=outputstu,
                        inputs=batch_data.pos,
                        grad_outputs=torch.ones_like(outputstu),
                        # 保留构图
                        retain_graph=True,  # 不释放
                        create_graph=True
                    )[0]
                    #outputteh = load_net(z=batch_data.z, pos=batch_data.pos, batch=batch_data.batch)
                    outputteh = load_net(batch_data)
                    forceteh = -torch.autograd.grad(
                        outputs=outputteh,
                        inputs=batch_data.pos,
                        grad_outputs=torch.ones_like(outputteh),
                        # 保留构图
                        retain_graph=True,
                        create_graph=True# 不释放
                    )[0]
                    target_loss += (1/p)*loss_f(outputteh, outputstu) + loss_f(forceteh, forcestu)
                    #target_loss += loss_f(outputteh, outputstu) + loss_f(forceteh, forcestu)
                target_loss = target_loss / len(dataloder)
                param_loss = torch.nn.functional.mse_loss(student_params_list[-1][base_model_params_mask], target_expert_params,
                                                          reduction="sum")

                #param_dist += torch.nn.functional.mse_loss(start_expert_params[base_model_params_mask],target_expert_params, reduction="sum") # 放大器
                #param_loss /= (param_dist+1e-9)
                total_loss = target_loss + param_loss
                ...
                logger.info(f'tota_loss:{total_loss.detach().cpu().item()}')
                if self.optimizer_adapter is not None:
                    param_loss.backward()
                    ...
                    adapter_params_list.append(adapter_params)
