...
class ReparamModule(nn.Module):
    ...
    def __init__(self, module):
        ...
        self.module = module
        #self._original_module = copy.deepcopy(module)
    ...
    # def to_original_model(self, flat_param=None):
    #     """
    #     根据当前flat_param或给定flat_param，把模型还原成一个独立的nn.Module。
    #     """
    #     if flat_param is None:
    #         flat_param = self.flat_param.detach()
    #
    #     # 先深拷贝一份原始module
    #     import copy
    #     restored_model = copy.deepcopy(self._original_module)
    #
    #     # 把 flat_param 按原始 param_shapes 拆分
    #     ps = (t.view(s) for (t, s) in zip(flat_param.split(self._param_numels),
    #                                       self._param_shapes))
    #
    #     # 依次赋值回 restored_model 的参数
    #     for (mn, n), p in zip(self._param_infos, ps):
    #         setattr(self._get_module_from_name_in(restored_model, mn), n,
    #                 nn.Parameter(p.clone()))
    #
    #     # 共享参数也要赋值
    #     for (mn, n, shared_mn, shared_n) in self._shared_param_infos:
    #         setattr(self._get_module_from_name_in(restored_model, mn), n,
    #                 getattr(self._get_module_from_name_in(restored_model, shared_mn), shared_n))
    #
    #     return restored_model
    #
    # def _get_module_from_name_in(self, module, mn):
    #     """在指定module中按名字找到子模块"""
    #     if mn == '':
    #         return module
    #     m = module
