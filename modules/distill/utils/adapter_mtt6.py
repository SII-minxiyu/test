    ...
    def distill(
        ...
        with tqdm(range(0, num_iteration)) as pbar:
            for distill_it in pbar:
                ...
                # if data_mask_rate > 0.:
                #     data_indices_list_len = len(data_indices_list)
                #     retain_indices = np.asarray(sorted(
                #         random.sample(
                #             range(0, data_indices_list_len),
                #             int(data_indices_list_len * (1 - data_mask_rate))
                #         )
                #     ))
                #     sampler.set_indices((np.asarray(data_indices_list)[retain_indices]).tolist())
                # else:
